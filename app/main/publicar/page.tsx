...
import { TrendingUp, TrendingDown, BookOpen, ChevronDown } from 'lucide-react'
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        ...
        >
          <TrendingUp
            size={24}
            className={`mx-auto mb-2 ${tipo === 'movimentacao' ? 'text-brand-green' : 'text-brand-muted'}`}
          />
          <p className={`text-sm font-semibold ${tipo === 'movimentacao' ? 'text-brand-green' : 'text-brand-muted'}`}>
            Movimentação
          </p>
          <p className="text-xs text-brand-muted mt-0.5">Compra ou venda</p>
        </button>
        ...
        >
          <BookOpen
            size={24}
            className={`mx-auto mb-2 ${tipo === 'tese' ? 'text-purple-400' : 'text-brand-muted'}`}
          />
          <p className={`text-sm font-semibold ${tipo === 'tese' ? 'text-purple-400' : 'text-brand-muted'}`}>
            Tese
          </p>
          <p className="text-xs text-brand-muted mt-0.5">Análise e estratégia</p>
        </button>
      </div>
      ...
      >
          ...
          <>
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: Bitcoin, PETR4, AAPL..."
                className="input"
              />
            </div>
        ...
        <div>
          ...
          <textarea
            ...
            placeholder={
              tipo === 'tese'
                ? 'Descreva sua análise, thesis, motivação para esta tese...'
                : 'Por que você fez esta operação? Qual sua tese?'
            }
          ...
          <p className="text-xs text-brand-muted mt-1 text-right">
            {conteudo.length}/{tipo === 'tese' ? 2000 : 500}
          </p>
        </div>
